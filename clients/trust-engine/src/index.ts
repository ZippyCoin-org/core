...
// Validation schemas
const trustFieldSchema = z.object({
  field_name: z.string(),
  field_type: z.enum(['Numeric', 'Boolean', 'Categorical', 'TimeSeries', 'CompoundMetric']),
  weight: z.number().min(0).max(1),
  data_source: z.object({
    type: z.enum(['OnChain', 'OffChain', 'UserInput', 'CoreTrust', 'CrossReference']),
    contract_address: z.string().optional(),
    api_endpoint: z.string().optional(),
    ref_field: z.string().optional(),
  }),
  validation_method: z.enum(['Range', 'Cryptographic', 'Peer', 'Historical', 'Performance', 'AI', 'Custom']),
...
const customTrustMetricsSchema = z.object({
  app_id: z.string(),
  developer_id: z.string(),
  custom_fields: z.record(trustFieldSchema),
  aggregation_rules: z.object({
    method: z.enum(['WeightedAverage', 'WeightedSum', 'Minimum', 'Maximum', 'Custom']),
    core_trust_weight: z.number().min(0).max(1),
    custom_weight: z.number().min(0).max(1),
  }),
  validation_rules: z.object({
    required_fields: z.array(z.string()),
    min_core_score: z.number().min(0).max(1),
    max_decay_rate: z.number().min(0).max(1),
  }),
});
...
class EnhancedTrustEngine {
  ...
  private trustCalculators: Map<string, any> = new Map();
  ...
  private async calculateCustomScore(walletAddress: string, appId: string): Promise<number> {
    try {
  ...
  private async getFieldValue(walletAddress: string, appId: string, fieldName: string, field: TrustField): Promise<number> {
    try {
      // Check cache first
      ...
      switch (field.data_source.type) {
        ...
        case 'OffChain':
          try {
            ...
            if (resp.ok) {
              ...
              const candidate = typeof body === 'number' ? body : ((body as any)?.value ?? (body as any)?.data?.value);
        ...
        case 'UserInput':
          // Get from database
          const result = await this.db.query(
            'SELECT field_value FROM custom_trust_scores WHERE wallet_address = $1 AND app_id = $2 AND field_name = $3',
            [walletAddress, appId, fieldName]
          );
          if (result.rows.length > 0) {
        ...
        case 'CrossReference':
          try {
            if (field.data_source.ref_field === 'core_trust') {
              ...
              const result = await this.db.query(
                'SELECT field_value FROM custom_trust_scores WHERE wallet_address = $1 AND app_id = $2 AND field_name = $3',
                [walletAddress, appId, field.data_source.ref_field]
              );
              if (result.rows.length > 0) {
  ...
  private normalizeFieldValue(value: number, field: TrustField): number {
  ...
  async loadCustomMetrics(): Promise<void> {
    try {
...
app.get('/api/v1/trust/metrics/:appId', async (req: Request, res: Response) => {
  try {
